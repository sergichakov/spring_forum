...
public class DirectoryRestController {
    ...
    public DeferredResult<ResponseEntity<DirectoryRest>>
            addDirectory(@RequestBody DirectoryWebDto directoryRest) throws ExecutionException, InterruptedException {

    	LOGGER.info("Start");
    	LOGGER.debug("Creating Directory");
		LOGGER.info("Thread : " + Thread.currentThread());
		DeferredResult<ResponseEntity<DirectoryRest>> deferredResult = directoryWebService.createDirectory(directoryRest);
        LOGGER.info("Ending");
        return deferredResult;
    }
    ...
    @RequestMapping(value = "/directories/maxtopic", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    public DeferredResult<ResponseEntity<Long>> getMaxTopicIdFromDirectory() { // String Id
        LOGGER.info("Start");
        LOGGER.debug("Get maxTopic from directories table");
        LOGGER.info("Thread : " + Thread.currentThread());
        DeferredResult<ResponseEntity<Long>> deferredResult = directoryWebService.getMaxTopicId();
        LOGGER.info("Ending");
        return deferredResult;
    }
}
